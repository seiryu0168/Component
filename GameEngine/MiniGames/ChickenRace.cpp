...
void ChickenRace::Initialize()
{
	...
	//â¬ïœÇ…Ç≈Ç´ÇÈÇÊÇ§Ç…å„Ç≈Ç∑ÇÈ
	Players_ = 2;

	//óêêîÇê∂ê¨ÇµÅAñ⁄ïWéûä‘ÇåàíËÇ∑ÇÈ
	{
	...
	Text text;
	std::string str = "ñ⁄ïWéûä‘ : " + std::to_string(TargetTime) + "ïb";
	text.SetText(str);
...
void ChickenRace::SendTime(Object* target, float time)
{
	...
	for (auto&& itr : childList_)
	{
		if (itr.get() == target)
		{
			...
			{
				Text text;
				text.SetText(std::format("Player{:d} : {:g}ïb", element + 1 ,PlayersTime_[element]));
				switch (element)
				{
				case 0:
...
void ChickenRace::Finish()
{
	...
	//text_->SetText(std::format("Player1 : {:g}ïb  Player2 : {:g}ïb", PlayersTime_[0], PlayersTime_[1]));
}
