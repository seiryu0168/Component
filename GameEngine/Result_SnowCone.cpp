...
void Result_SnowCone::Initialize()
{
	...
	Text scoreText("íÒãüÇ≈Ç´ÇΩêî 0", "ÇËÇ¢ÇƒÇ™Ç´ïM", { 0,0,500,50 });
...
void Result_SnowCone::Show()
{
	if ((interval_ * count_) <= time_->GetSeconds<float>() - 1.0f)
	{
		Create(count_);
		count_++;
		GetComponent<Text>(resultTextNum_).SetText("íÒãüÇ≈Ç´ÇΩêî " + std::to_string(count_));

	}
	if (count_ > score_)
	{
		GetComponent<Text>(resultTextNum_).SetText("íÒãüÇ≈Ç´ÇΩêî " + std::to_string(score_));
		time_->Lock();
		ShowCommand();
